...
describe('CoverImage', () => {
  it('renders image when src provided', () => {
    render(
      <CoverImage src="https://example.com/cover.jpg" alt="Test cover" type="BOOK" variant="grid" />
    );
  ...
  it('renders fallback icon when no src', () => {
    render(<CoverImage alt="Test cover" type="BOOK" variant="grid" />);
  ...
  it('shows correct icon for each type', () => {
    const { rerender } = render(<CoverImage alt="Test" type="BOOK" variant="grid" />);
    expect(screen.getByText('📚')).toBeInTheDocument();
